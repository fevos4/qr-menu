...
export default function AdminPage() {
  ...
  <div className="bg-white rounded-lg p-6 flex-1">
       <Folder className="text-orange-400" size={32} />
          ...
          <p className="text-4xl font-bold text-stone-800">{categories.length}</p>
        </div>
         ...
         <div className="bg-white rounded-lg p-6 flex-1">
          ...
          <p className="text-4xl font-bold text-stone-800">{menuItems.filter(item => item.isAvailable).length}</p>
        </div>
      </div>
    </div>
  )
